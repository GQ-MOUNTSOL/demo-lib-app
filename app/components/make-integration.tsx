...
export function MakeIntegration() {
 ...
 const MAKE_WEBHOOK_URL ="https://hook.eu2.make.com/vj9cf3va2p8bmil6ay30j7buwlapufhs" // testing scenario webhook
  ...
  const prompt1 = `Act as a highly skilled literary analyst, deep narrative thinker, and expert in chapter-by-chapter breakdowns. When I give you a book title, provide an extremely detailed, immersive, and structured summary that is no less than 6000 words. The goal is to make the summary as valuable, comprehensive, and educational as reading the original book. Start with a brief overview of the book's context, author, genre, and overall message or contribution. Help readers understand what the book is and why it matters. Then, for each chapter, write full paragraphs (not bullet points) that unpack all major events, arguments, character developments, and transitions. Include key quotes from the book (using actual text where possible) to provide authenticity and literary richness. Analyze the purpose and implications of each chapter—what themes does it explore? How does it develop the book's message? Explain complex ideas in simple terms, so readers of all backgrounds can understand and appreciate the material. After the chapters, analyze overarching themes, motifs, symbols, or stylistic choices. Summarize the personal, philosophical, business, or societal takeaways from the book—what can the reader apply or reflect on? Why is the book still relevant? Finally, wrap up with a conclusion that reflects on the emotional and intellectual value of the book, its legacy, and what makes it memorable. Use clear, intelligent, and engaging language. Avoid vague generalities or fluff. Only mention the informaiton, not the thoughts`

  const prompt4 = `Act as a book metadata expert, providing comprehensive details on any book title given. When provided with a book name, return structured information covering the following aspects: 1. **Book Title and Subtitle** (if applicable). 2. **Primary Genre** and **Subgenres** – Categorize the book appropriately. 3. **Author Information**: - Full name - Nationality and time period - A brief biography (one or two sentences). If unavailable, state Information unavailable. 4. **Similar Books** in the same genre or by the same author. 5. **Famous Reviews** from reputable sources (can be synthetic but realistic).6. **Multilingual Information** – Provide the book title, author name, and genre classification in **Urdu, Arabic, German, and Spanish** for broader accessibility. 7. Ensure accuracy and avoid any hallucinations. If certain details are unavailable, clearly state that rather than making assumptions. The output should be well-organized, informative, and structured professionally.`
